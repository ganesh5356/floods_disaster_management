...
const DdmResources: React.FC<DdmResourcesProps> = ({ resources, resourceRequests, setResourceRequests }) => {
    const { user } = useAuth();
    const [isModalOpen, setModalOpen] = useState(false);
    const { register, handleSubmit, reset } = useForm<RequestFormData>();

    // Filter resources for the current DDMA's district
    // Simplified logic: assumes DDMA is under a specific SDMA, which needs to be looked up.
    // For this mock, let's assume DDMA's resources are those in their state not assigned elsewhere.
    const sdmaId = user?.id.replace('DDMA', 'SDMA'); // Simplistic mapping
    const ddmaResources = resources.filter(r => r.assignedSdm === sdmaId); // This logic is likely too simple. A better approach would be to have resources assigned directly to districts or have a clear hierarchy. For now, we'll assume SDMA resources are available to their DDMAs.
    const ddmaRequests = resourceRequests.filter(r => r.requesterId === user?.id);

    const total = ddmaResources.reduce((acc, r) => acc + r.quantity, 0);
    const available = ddmaResources.reduce((acc, r) => acc + r.available, 0);
    ...
    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-2xl font-bold text-gray-900">District Resource Overview ({user?.district})</h2>
                <p className="text-gray-600">Manage resources available in your district.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-2xl shadow-lg">
                    ...
                    <div className="overflow-x-auto max-h-96">
                        <table className="w-full text-sm text-left text-gray-600">
                            ...
                            <tbody>
                                {ddmaResources.length > 0 ? ddmaResources.map(r => (
                                    <tr key={r.id} className="bg-white border-b hover:bg-gray-50">
                                     ...
                                     <tr>
                                        <td colSpan={3} className="text-center py-8 text-gray-500">No resources available in this district.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-xl font-semibold text-gray-800">My Requests to SDMA</h3>
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {ddmaRequests.length > 0 ? ddmaRequests.map(req => (
            ...
            <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} title="Request New Resources from SDMA">
